...
tesselate a rectangle with sides parallel to the coordinate axes.
...
class Rectangular_Grid(Grid):
    ...
    def configure_topology(self, i, j):
        ...
        north = self[i+1, j]
        if north: cell.topology["north"] = north
        south = self[i-1, j]
        if south: cell.topology["south"] = south

        east = self[i, j+1]
        if east: cell.topology["east"] = east
        west = self[i, j-1]
        if west: cell.topology["west"] = west
