...
# -*- coding: utf-8 -*-
##############################################################################
# sparsify_demo.py - demonstrate the sparsify by braiding algorithm
# Eric Conrad
...
def tweak(fig, axs):
    ...
    for j in range(3):
        titles = ["Full Maze", "Sparse Maze", "Sparser Maze"]
        ax = axs[j]
        ax.title.set_text(titles[j])
        ax.set(aspect=1)
        ax.axis('off')
    fig.suptitle("Sparsifying Demonstration")
...
def render(maze, filename, p=0.5, tweaker=tweak):
    ...
    Aldous/Broder algorithm.  Three subplots are produced:
        ...
        b) a plot of the maze after single-pass braid-removal by
           sparsification
        c) a plot of the maze after a second partial sparsification pass

    maze = a Grid object with a maze to sparsify
    ...
    layout.draw_grid()
    print("   column 1 - first pass, full sparsification...")
    Braiding.sparsify(maze)
    ax = axs[1]
    layout = Color_Layout(maze, plt, figure=[fig, ax])
    layout.draw_grid()
    print("   column 2 - second pass, partial at %f..." % (p * 100))
    Braiding.sparsify(maze, bias = p)
    ax = axs[2]
    layout = Color_Layout(maze, plt, figure=[fig, ax])
    layout.draw_grid()
...
if __name__ == "__main__":
    print("Sparse Maze Test Script...")
    maze = make_rectangular_grid(30, 20)
    generate_maze(maze)
    filename = "demos/sparsify-array.png"
