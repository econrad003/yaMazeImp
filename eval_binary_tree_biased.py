...
# -*- coding: utf-8 -*-
##############################################################################
# eval_binary_tree_biased.py - evaluate the binary tree implementation
# Eric Conrad
...
This differs from eval_binary_tree.py using a bias of 75% instead of a 50%
bias.  This corresponds with replacing a fair coin (50% heads) with an
unfair coin (75% heads).  The effect is to increase the expected length
of horizontal runs.
...
rows, cols = 20, 30
runs = 20
filename = "stats/eval_binaryTree_biased.txt"
title = "Binary Tree algorithm (N/E)"
parameters = "Rectangular_Grid(%d, %d), bias=75%%" % (rows, cols)
...
with open(filename, 'w') as fp:
    ...
    fp.write("    Top row is a horizontal run.\n")
    fp.write("    Last column is a vertical rise.\n")
    fp.write("    longer average horizatal runs than unbiased binary" \
             + " tree.\n")
print("Saved to " + filename)

# END: eval_binary_tree_biased.py
