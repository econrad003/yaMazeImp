...
# -*- coding: utf-8 -*-
##############################################################################
# eval_binary_tree2.py - evaluate the alternate binary tree implementation
# Eric Conrad
...
This evaluates an alternative binary tree implementation (not the binary
tree algorithm of [1]) on rectangular grids.

At the moment, I am not certain that the algorithm always produces a
binary spanning tree for a rectangular grid.  (It will always produce
a binary spanning forest on a connected grid.  But there is no guarantee
that the forest will be connected to form a single binary spanning tree.
There are connected grids for which a spanning tree is not guaranteed,
for example, a cylinder grid.)
...
rows, cols = 20, 30
runs = 20
filename = "stats/eval_binaryTree2.txt"
title = "Binary Tree algorithm (non-orthogonal)"
...
with open(filename, 'w') as fp:
    ...
    fp.write("    mu(v, e)=(%d, %d) exactly!\n" \
        % (rows * cols, rows * cols - 1))
    fp.write("    mu(k)=1 most of the time.  (Perhaps every time?  Proof?)\n")
print("Saved to " + filename)

# END: eval_binary_tree2.py
